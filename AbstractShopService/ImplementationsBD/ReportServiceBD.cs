...
namespace AbstractShopService.ImplementationsBD
{
    public class ReportServiceBD : IReportService
    {
        ...
        public List<BonusFinesLoadViewModel> GetBonusFinesLoad()
        {
            return context.BonusFines
                            .ToList()
                            .GroupJoin(
                                    context.BonusFinePayments
                                                .Include(r => r.Teacher)
                                                .ToList(),
                                    BonusFine => BonusFine,
                                    BonusFinePayment => BonusFinePayment.BonusFine,
                                    (BonusFine, BonusFineCompList) =>
            new BonusFinesLoadViewModel
            {
                BonusFineName = BonusFine.BonusFineName,
                TotalCount = BonusFineCompList.Sum(r => r.Count),
                Components = BonusFineCompList.Select(r => new BonusFinesPaymentLoadViewModel { PaymentName = r.Teacher.TeacherFIO, Count = r.Count }).ToList()
            })
                            .ToList();
        }
            ...
            try
            {
                ...
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        excelcells.Value2 = elem.BonusFineName;
        ...
        public List<TeacherZakazsModel> GetTeacherZakazs(ReportBindingModel model)
        {
            return context.Zakazs
                            .Include(rec => rec.Teacher)
                            .Include(rec => rec.Section)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new TeacherZakazsModel
                            {
                                TeacherName = rec.Teacher.TeacherFIO,
        ...
        public void SaveTeacherZakazs(ReportBindingModel model)
        {
            ...
            //заполняем таблицу
            var list = GetTeacherZakazs(model);
            var fontForCells = new iTextSharp.text.Font(baseFont, 10);
            for (int i = 0; i < list.Count; i++)
            {
                cell = new PdfPCell(new Phrase(list[i].TeacherName, fontForCells));
                table.AddCell(cell);
